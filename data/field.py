...
class BBoxField(RawField):
    def __init__(self, preprocessing=None, postprocessing=None, detections_path=None, max_detections=100,
        ...
        super(BBoxField, self).__init__(preprocessing, postprocessing)
...
class RelPairField(RawField):
    def __init__(self, preprocessing=None, postprocessing=None, detections_path=None, max_detections=100,
        ...
        super(RelPairField, self).__init__(preprocessing, postprocessing)
    ...
    def preprocess(self, x, avoid_precomp=False):
        ...
        try:
            ...
            warnings.warn('Could not find detections for %d' % image_id)
            return
        # print(rel_pairs, rel_pairs.shape)
        delta = self.max_detections * (self.max_detections - 1) - rel_pairs.shape[0]
        if delta > 0:
            rel_pairs = np.concatenate([rel_pairs, -np.ones((delta, rel_pairs.shape[1]))], axis=0)
        elif delta < 0:
            rel_pairs = rel_pairs[:self.max_detections * (self.max_detections - 1)]
        return rel_pairs.astype(np.int)


class RelLabelField(RawField):
    def __init__(self, preprocessing=None, postprocessing=None, detections_path=None, max_detections=100,
                 sort_by_prob=False, load_in_tmp=True):
        self.max_detections = max_detections
        self.detections_path = detections_path
        ...
        super(RelLabelField, self).__init__(preprocessing, postprocessing)
    ...
    def preprocess(self, x, avoid_precomp=False):
        ...
        try:
            ...
            warnings.warn('Could not find detections for %d' % image_id)
            return
        delta = self.max_detections * (self.max_detections - 1) - rel_labels.shape[0]
        if delta > 0:
            # pad -1
            rel_labels = np.concatenate([rel_labels, -np.ones((delta, rel_labels.shape[1]))], axis=0)
        elif delta < 0:
            rel_labels = rel_labels[:self.max_detections * (self.max_detections - 1)]

        return rel_labels.astype(np.int)
