...
from models.beam_search import BeamSearchR
import pdb


class CaptioningModelR(Module):
    def __init__(self):
        super(CaptioningModelR, self).__init__()
    ...
    def forward(self, images, seq, bboxes=None, rel_pairs=None, rel_labels=None, *args):
        ...
        for t in range(seq_len):
            out, state = self.step(t, state, out, images, seq, bboxes, rel_pairs, rel_labels, *args, mode='teacher_forcing')
            outputs.append(out)
    ...
    def beam_search_r(self, visual: utils.TensorOrSequence, max_len: int, eos_idx: int, beam_size: int, out_size=1,
                    return_probs=False, **kwargs):
        bs = BeamSearchR(self, max_len, eos_idx, beam_size)
